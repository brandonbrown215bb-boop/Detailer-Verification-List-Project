...
| 🧪 **[`run-tests.bat`](run-tests.bat)** | **Run Automated Tests** (xUnit + Core coverage gate + focused Node suites) | Execute test suites |
| 📦 **[`publish-release.bat`](publish-release.bat)** | **Publish Production Releases** to `publish\` (explicit SemVer required) | `publish-release.bat 1.0.0` |
...
| [`run-tests.bat`](run-tests.bat) | Executes the xUnit suite through the measured Core coverage gate and focused Node.js suites. | `npm run test:coverage` + Node scripts |
| [`publish-release.bat`](publish-release.bat) | Generates self-contained production distributions and a Velopack installer in `publish\AHUVerification`, `publish\RuleEditor`, and `Releases\`. | `publish-release.bat <SemVer>` |
...
│
├── resources/
│   ├── bin/                          # Native decompressor binaries used by the test/package fixture
...
The licensed native decompressor binaries are maintained under `src/backend/AHUVerification.App/resources/bin/` and copied into the main desktop host output during build/publish.
...
- **.NET SDK**: [.NET 8 SDK](https://dotnet.microsoft.com/download) with the `net8.0` / `net8.0-windows` targeting packs
- **Node.js**: Node.js 22.18.x and `npm` (the repository pin is recorded in `.nvmrc`; `scripts\init_env.bat` checks it)
...
The xUnit command used by `run-tests.bat` is `npm run test:coverage`; it writes a Cobertura report and TRX log under ignored `TestResults/coverage-core/` and enforces the critical-module floors in [`docs/operations/coverage-baseline.json`](docs/operations/coverage-baseline.json). Browser and WebView2 smoke checks remain separate evidence classes.

### 5. Packaging & Deploying Releases
1. Run [`publish-release.bat`](publish-release.bat) with the version to release (for example, `publish-release.bat 1.0.0`). The version must match [`version.json`](version.json).
