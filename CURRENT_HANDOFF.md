...
## Phase 5 Clean-checkout Reproducibility Gate Completed — 2026-09-05

Phase 5 Slices P5-A through P5-D are complete locally, and the release-package preflight has reproduced from a clean Git-backed worktree. The code snapshot below matches `origin/master`; this handoff and the remediation plan contain the deliberate local evidence updates from this acceptance run.

- **Protected snapshot:** `HEAD 29836826b5cccedc8c2f3779b5dd8d5a35e19c22` on `master`, matching `origin/master`. Do not reset or clean this snapshot; generated package output remains ignored and has not been published.
...
- **Current results:** Version, TypeScript, the full frontend suite, dist/bundle/graph checks, focused bridge/DVL tests, solution build, production build, two-generation rule-pack byte determinism, and the pinned-runtime coverage gate passed. `npm run test:coverage` passed 205/205 C# tests and every named floor. The self-contained `win-x64` publishes and the 18-test Playwright suite passed; the one timing-sensitive narrow case passed 3/3 when isolated. `dotnet format AHUVerification.sln --verify-no-changes --no-restore --severity warn --verbosity minimal` exits 0. The final `publish-release.bat 1.0.0` run passed all pre-package gates in the clean Git-backed worktree and produced the setup executable, portable archive, Rule Editor archive, Velopack metadata, two SBOMs, and a nine-file SHA-256 manifest; independent hash and JSON parsing checks passed.
- **Clean-checkout evidence:** Temporary Git worktree `C:\Users\brand\AppData\Local\Temp\DVL-phase5-worktree-20260905-200440` was clean before and after verification. Its publisher reproduced the full package path, and both packaged executables opened responsively with their intended main-window titles.
- **Native launch boundary:** The restricted sandbox previously showed error dialogs because its identity has read-only access to the normal per-user WebView2 data root; WebView2 Runtime `152.0.4191.62` is installed. Native computer-use surfaces expose no desktop apps in this session, so no installer UI, UPZ file-dialog, save/reopen, Excel export, or uninstall interaction is claimed.
...
- **Next safe action:** On an authorized normal Windows desktop, run and record install/first launch, offline/native UPZ import, Rule Editor edit/publish/reload, save-close-reopen, final Excel export, and uninstall. Do not lower coverage floors, waive restricted-mode failures, or treat the generated package as published. External CI/branch-protection changes, push/publication, code signing, and live release remain outside this handoff.
